...
### 2.1. Diagrama de Blocos da Arquitetura

```mermaid
flowchart TD
    subgraph CAMADA_FISICA["Camada IoT & Automação Física (ESP32-S3 / FreeRTOS)"]
        Botao["Botoeira Industrial (GPIO 6)"] -->|Debounce 50ms / Toggle| ESP["ESP32-S3 (MCU Dual-Core)"]
        ESP -->|Comutação TIP122 (GPIO 7)| Motor["Motor DC da Esteira (5V-12V)"]
        ESP -->|Pulso 1000ms / Alerta (GPIO 2)| LED["LED Vermelho Difuso"]
        ESP -->|Padrão Sonoro Pulsado (GPIO 1)| Buzzer["Buzzer Piezoelétrico"]
    end

    subgraph COMUNICACAO["Enlace Físico Cabeado (UART TTL 3.3V / 115200 bps)"]
        ESP -->|UART TX GPIO 17 -> RX GPIO 15 / 'SYS_ON', 'SYS_OFF'| Pi["Raspberry Pi 5 (8 GB)"]
        Pi -->|UART TX GPIO 14 -> RX GPIO 18 / 'STOP', 'START', 'DEFECT', 'ALERTA'| ESP
    end

    subgraph VISAO_BORDA["Camada Cognitiva de Borda (Raspberry Pi 5 / Linux)"]
        PiCam["Pi Camera CSI-2 (1296x972 @ 60fps)"] -->|Buffer DMA RAM| CamModule["Módulo Câmera (Picamera2 / GStreamer)"]
        CamModule -->|Frame BGR NumPy| Motion["Detector de Movimento (MOG2 / ROI)"]
        Motion -->|Gatilho de Parada| Pipeline["Pipeline de Inspeção"]
        Pipeline -->|Recorte ROI 224x224| YOLO["YOLOv8n-cls (weights/best.pt)"]
    end

    subgraph SUPERVISAO["Camada de Persistência & Supervisão (Docker / Nginx)"]
        Pipeline -->|Transações ACID / WAL| DB[("SQLite (vinagrete.db)")]
        Pipeline -->|Gravação de Fotos de Refugo| Uploads["Armazenamento Local (/uploads)"]
        DB <-->|Consultas Assíncronas| API["FastAPI Backend (Porta 8000)"]
        API <-->|Proxy Reverso / Dashboard| UI["Interface Web Nginx (Porta 80)"]
        UI --- Operador["Operador Fabril"]
    end
```

### 2.2. Diagrama de Estados do Sistema
